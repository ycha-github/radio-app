...
import { FormRecursos } from '../../components/formCat/FormRecursos';
...
export const RecursoCompras = () => {
  const { events, setActiveEvent, startLoadingEvents, deleteEvent,user} = useRecursosStore();
...
const columns = [
  ...
  { field: 'estatus', type: 'boolean',headerClassName: "super", headerName: 'Estatus', flex: 1, minWidth: 90 },
  {
    ...
    getActions: (evento) => [
      ...
      aria-label="delete"
      >
        <Switch color='warning' checked={evento.row.estatus} name="estatus" onChange={(event)=>handleChange(event, evento.row.idrecursoCompras)} />
     </IconButton> 
    ], 
  },
];
  ...
  return (
    <>
     <h2 className='colorCat'>RECURSOS COMPRAS</h2>
    ...
    <div style={{ flexGrow: 1 }}>
      ...
      <StripedDataGrid
        ...
        getRowId={(row) => row.idrecursoCompras}
